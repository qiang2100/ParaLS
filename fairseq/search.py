...
class BeamSearch(Search):
    ...
    def step1(
        ...
        beams_buf = indices_buf // vocab_size
        #indices_buf = indices_buf.fmod(vocab_size)
        indices_buf = indices_buf-(beams_buf)*vocab_size
        # At this point, beams_buf and indices_buf are single-dim and contain relative indices
        return scores_buf, indices_buf, beams_buf
    ...
    def step2(
        ...
        top_prediction = torch.topk(lprobs[0][0], beam_size)

        #pdb.set_trace()


        scores_buf = top_prediction[0].view(1,-1)
        indices_buf = top_prediction[1].view(1,-1)
        # Project back into relative indices and beams
        index = [ i for i in range(beam_size)]

        beams_buf = torch.tensor(index)
    ...
    def step3(
        ...
        this_tokens_labels=None,
        this_again_tokens_labels=None
    ):
        ...
        #top_prediction = torch.topk(lprobs.view(beam_size,-1), 1, dim=1)
        if suffix_id!=[] and prev_suffix_id!=[] and prev_again_suffix_id!=[]:
            top_prediction=[]

            now_scores=lprobs.view(beam_size,-1)[:,suffix_id[0]]
            now_indices=torch.tensor([suffix_id[0]]*beam_size).to("cuda")

            prev_again_scores=lprobs.view(beam_size,-1)[:,prev_again_suffix_id[0]]
            prev_again_indices=torch.tensor([prev_again_suffix_id[0]]*beam_size).to("cuda")


            prev_scores=lprobs.view(beam_size,-1)[:,prev_suffix_id[0]]
            prev_indices=torch.tensor([prev_suffix_id[0]]*beam_size).to("cuda")


            # final_indices=now_indices
            # final_indices=torch.
            final_indices=torch.where(this_tokens_labels,prev_indices,now_indices)   
            final_indices=torch.where((this_again_tokens_labels*this_tokens_labels),prev_again_indices,final_indices)

            final_scores=torch.where(this_tokens_labels,prev_scores,now_scores)   
            final_scores=torch.where((this_again_tokens_labels*this_tokens_labels),prev_again_scores,final_scores)

            top_prediction.append(final_scores)
            top_prediction.append(final_indices)

        elif prev_suffix_id==[] and suffix_id!=[]:
            top_prediction=[]
            top_scores,top_indices=torch.topk(lprobs.view(beam_size,-1), 1, dim=1)
            top_scores=top_scores.view(1,-1)
            top_indices=top_indices.view(1,-1)

            now_scores=lprobs.view(beam_size,-1)[:,suffix_id[0]].view(1,-1)
            now_indices=torch.tensor([suffix_id[0]]*beam_size).to("cuda").view(1,-1)  
                 
            # if this_again_tokens_labels!=None:
            final_indices=torch.where(this_tokens_labels,top_indices,now_indices)   
            final_scores=torch.where(this_tokens_labels,top_scores,now_scores)
            # else:
            #     final_indices=now_indices
            #     final_scores=now_scores  
        ...
        elif prev_suffix_id!=[] and prev_again_suffix_id==[] and suffix_id!=[]:
            ...
            prev_scores=lprobs.view(beam_size,-1)[:,prev_suffix_id[0]].view(1,-1)
            prev_indices=torch.tensor([prev_suffix_id[0]]*beam_size).to("cuda").view(1,-1)

            final_indices=torch.where(this_tokens_labels,prev_indices,now_indices)  
            final_indices=torch.where((this_again_tokens_labels*this_tokens_labels),top_indices,final_indices)

            final_scores=torch.where(this_tokens_labels,prev_scores,now_scores)   
            final_scores=torch.where((this_again_tokens_labels*this_tokens_labels),top_scores,final_scores)
            ...
            top_indices_copy=copy.deepcopy(top_indices)

            prev_scores=lprobs.view(beam_size,-1)[:,prev_suffix_id[0]].view(1,-1)
            prev_indices=torch.tensor([prev_suffix_id[0]]*beam_size).to("cuda").view(1,-1)

            final_indices=torch.where(this_tokens_labels,prev_indices,top_indices)  
            final_indices=torch.where((this_again_tokens_labels*this_tokens_labels),top_indices_copy,final_indices)

            final_scores=torch.where(this_tokens_labels,prev_scores,top_scores) 
            final_scores=torch.where((this_again_tokens_labels*this_tokens_labels),top_scores_copy,final_scores) 
            
            top_prediction.append(final_scores)
            top_prediction.append(final_indices)

        elif prev_suffix_id!=[] and prev_again_suffix_id!=[]:
            ...
            prev_again_scores=lprobs.view(beam_size,-1)[:,prev_again_suffix_id[0]].view(1,-1)
            prev_again_indices=torch.tensor([prev_again_suffix_id[0]]*beam_size).to("cuda").view(1,-1)
            ...
            final_indices=torch.where((this_again_tokens_labels*this_tokens_labels),prev_again_indices,top_indices)  

            final_scores=torch.where((this_again_tokens_labels*this_tokens_labels),prev_again_scores,top_scores)            
            top_prediction.append(final_scores)
            top_prediction.append(final_indices)  
        ...
        # if suffix_id!=[] and prev_suffix_id!=[]:
        #     top_prediction=[]

        #     now_scores=lprobs.view(beam_size,-1)[:,suffix_id[0]]
        #     now_incies=torch.tensor([suffix_id[0]]*beam_size).to("cuda")

        
        #     top_prediction.append(lprobs.view(beam_size,-1)[:,suffix_id[0]])
        #     top_prediction.append(torch.tensor([suffix_id[0]]*beam_size).to("cuda"))
    ...
    def step4(
        ...
        if id1==None:
            id1=1
        top_prediction = torch.topk(lprobs.view(beam_size,-1), id1, dim=1)

        scores_buf = top_prediction[0][:,-1].view(1,-1)
        indices_buf = top_prediction[1][:,-1].view(1,-1)
        # Project back into relative indices and beams
        index = [ i for i in range(beam_size)]
