...
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if 1==0:
            cosine_scores=self.SimCTG_cosine_simirality(net_output[1]["inner_states"][-1])
            margin=0.5
...
#   SimCTG loss function
    ...
    def compute_contrastive_loss(self,score_matrix, margin):
        ...
        cl_loss = torch.mean(loss_matrix)*loss_matrix.size()[0]*loss_matrix.size()[1]
        return cl_loss

    def SimCTG_cosine_simirality(self,outputs):
        outputs=outputs.transpose(0,1)

        last_hidden_state = outputs # torch.Size([3, 64, 768])
