...
bleurt_model = AutoModelForSequenceClassification.from_pretrained("bleurt-large-512").cuda()
bleurt_model.eval()
# bleurt_model=None
...
def substitutes_scores(original_sentence, substitutes, index_of_word):
    ...
    score_bart = bart_scorer.score(sss, ccs, batch_size=10)
    with torch.no_grad():
        input_dict=bleurt_tokenizer(sss, ccs, return_tensors='pt', padding=True)

        scores_bleurt=bleurt_model(input_ids=input_dict["input_ids"].cuda(),\
                                token_type_ids=input_dict["token_type_ids"].cuda(),\
                                attention_mask=input_dict["attention_mask"].cuda())[0].squeeze()

    #return score_bart,[],1
    #return [], scores_bleurt.tolist(),1
    return score_bart,scores_bleurt.tolist(),1
...
def lookahead_scores(model,original_sentence,substitutes,word_index):
    ...
    for i in range(len(substitutes)):
        ...
        candi_tokens=model.encode(" "+substitutes[i].strip())
        #tmp_suffix_tokens=candi_tokens[1:-1].tolist()+suffix_tokens
        tmp_suffix_tokens=candi_tokens[1:-1].tolist()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-gfc", "--golden_file_cadidates", type=str, help="path of the golden file dataset for gap",
                        default='data/LS07/lst.gold.candidates')
        ...
        for instance in reader.words_candidate[main_word]:
            for context in reader.words_candidate[main_word][instance]:
                ...
                for i in range(len(proposed_words_list)):
                    ...
                    score=ahead_scores[i]*0.04+bleurt_scores[i]+bart_scores[i]
                    #score=ahead_scores[i]*0.04+bleurt_scores[i]
                    #score=bart_scores[i]
                    #score=bleurt_scores[i]
