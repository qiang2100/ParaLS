...
def give_dynamic_embedding_scores(model,candis,prefix,ori_sentence,ori_index,layer_index=-1):
    ...
    ori_word_tokens=model.encode(ori_word)[:-1]
    ...
    if layer_index!=None:
        this_encoder_outs=encoder(this_input_tokens.unsqueeze(0).cuda(),return_all_hiddens=True)["encoder_states"][layer_index].cpu().squeeze()
        #this_ori_outs=this_encoder_outs[this_first_index:this_last_index].squeeze()
        this_ori_outs=torch.sum(this_encoder_outs,dim=0)/len(this_encoder_outs)
...
def give_real_scores_ahead_avg(tgt_dict,outputs,scores_with_suffix,scores_with_suffix_masks,suffix_tokens,prefix_len=None,prefix_str=None,max_ahead=1,flag=1):
    ...
    for i in range(len(beam_idx)):
        if sorted_scores[i]==(-math.inf):
            continue
        tmp_str1=tgt_dict.string(outputs[beam_idx[i],:(len_idx[i]+1)]).replace("@@ ","")
        tmp_str1_len=len_idx[i]-first_index+1+len(suffix_tokens)
